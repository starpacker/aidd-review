...
We conducted an empirical evaluation that extends this finding to drug discovery specifically. Using a gold standard dataset of 36 molecules (12 FDA-approved drugs, 12 clinical-stage failures, 12 computational decoys), we tested five systems spanning the automation spectrum: a pure LLM (Claude 4.6 Opus without tools), an LLM+Tools agent (Claude with RDKit-based chemistry tools in a ReAct loop), an RDKit composite scoring pipeline, an ADMET proxy scoring system, and the DruGUI eight-stage virtual screening pipeline (Table 3). All statistical comparisons use Mann-Whitney U tests with bootstrap 95% confidence intervals (n = 10,000 iterations).

**Table 3. Cross-system evaluation of clinical utility prediction.** Scores are mean ± SEM. AUC computed via Mann-Whitney U (scipy.stats); 95% CI via bootstrap resampling.

| System | A (Approved) | B (Failed) | C (Decoy) | Ranking | AUC (A vs B) | 95% CI | p-value |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Direct LLM | 5.82 ± 0.34 | 5.44 ± 0.30 | 5.08 ± 0.19 | **A > B > C** | 0.642 | [0.40, 0.86] | 0.248 |
| LLM + Tools Agent | 8.28 ± 0.22 | 4.94 ± 0.58 | 5.69 ± 0.39 | A > C > B | **0.948** | [0.85, 1.00] | **0.0002** |
| RDKit pipeline | 0.724 ± 0.052 | 0.653 ± 0.038 | 0.758 ± 0.013 | C > A > B | 0.694 | [0.46, 0.92] | 0.112 |
| ADMET proxy | 0.631 ± 0.040 | 0.681 ± 0.026 | 0.787 ± 0.005 | C > B > A | 0.410 | [0.18, 0.65] | 0.470 |
| DruGUI pipeline | 0.557 ± 0.027 | 0.496 ± 0.028 | 0.592 ± 0.009 | C > A > B | 0.667 | [0.42, 0.89] | 0.175 |

The results reveal a striking pattern we term the **Goodhart gradient** (Figure 4). The only system that correctly ranked approved drugs above clinical failures above decoys (A > B > C) was the pure LLM — the system with no computational chemistry tools at all. Its discrimination was weak (AUC 0.642, 95% CI [0.40, 0.86], p = 0.248), but directionally correct, likely reflecting chemical knowledge distilled from scientific literature during pretraining. Adding RDKit tools to the same LLM dramatically increased discrimination power (AUC 0.948, 95% CI [0.85, 1.00], p = 0.0002) but introduced a characteristic distortion: decoys scored higher than clinical failures (C > B), because the computational tools reward drug-likeness metrics that decoys are by construction designed to satisfy. The three pure computational pipelines exhibited progressively severe Goodhart inversion. RDKit and DruGUI both ranked C > A > B. The ADMET proxy showed the most extreme inversion — C > B > A — with an AUC of 0.069 for distinguishing approved drugs from decoys, meaning it assigned higher scores to decoys than to FDA-approved drugs with near-certainty (p < 0.001).

This gradient formalizes a manifestation of Goodhart's Law in drug discovery: as a system relies more heavily on computational metrics and less on external knowledge, its alignment with clinical utility systematically decreases (Spearman rho = -0.80, p = 0.104 across 5 systems). The LLM acts as a partial corrective — its knowledge of drug failure modes, clinical trial history, and pharmacological context provides a countervailing signal to the metric-optimizing tools. But this corrective is overwhelmed as tool dependence increases. The LLM+Tools agent achieves the best binary discrimination between approved and failed drugs (AUC 0.948) while simultaneously being a Goodhart victim (C > B) — the paradox of a system that can tell winners from losers but cannot tell losers from decoys.

A per-molecule failure detectability analysis reveals a subtler problem. When we test whether each clinical failure scores below the median across all three computational pipelines (RDKit, ADMET proxy, DruGUI), 11 of 12 failures (92%) are flagged by at least one system. Only DSP-1181, Sumitomo Pharma's first AI-designed drug (discontinued for undisclosed reasons), passes all computational screening filters. However, this apparent detection success is misleading: the computational systems flag these molecules for having poor drug-likeness scores, not for the biological mechanisms that actually caused their clinical failure. Rofecoxib's cardiovascular toxicity from off-target COX-1 inhibition, ximelagatran's idiosyncratic hepatotoxicity, semagacestat's Notch pathway effects, and REC-994's surrogate endpoint failure each represent biological failure modes that are orthogonal to the computational metrics being measured. The systems produce the right rejections for the wrong reasons — a distinction that matters critically when the goal is to predict failures prospectively rather than merely correlate with them retrospectively.
...
The algorithmic shift required is from pattern recognition to causal inference. Current AIDD models learn statistical correlations between molecular features and assay outcomes. They cannot distinguish a feature that causes activity from one that merely co-occurs with it in the training set. Causal inference frameworks — structural equation models, do-calculus, counterfactual reasoning in the tradition of Pearl [39] — offer a path toward models that represent biological mechanisms rather than statistical associations. The evidence for this approach is already compelling: Mendelian randomization studies demonstrate that genetically supported drug targets have approximately 2.6-fold greater probability of clinical success than unsupported targets [44]. When causal genetic features are integrated into machine learning classifiers rather than used as binary filters, the enrichment is even more striking — directly addressing the target hypothesis failures exemplified by BEN-2293.

Multi-modal data integration is the second algorithmic priority. Single-cell RNA sequencing across multiple diseases and tissues has shown that targets with cell-type-specific and disease-cell-specific expression support approximately triple the odds of reaching Phase III [45]. Combining genomic, transcriptomic, proteomic, and clinical outcome data into unified predictive models has the potential to capture the systems-level biology that single-modality models miss. Patient-derived organoids can serve as bridges between molecular data and clinical phenotypes. Real-world evidence from electronic health records and claims databases offers post-market training signals that can improve prospective prediction.

Explainable AI (xAI) is the third priority — not as an academic exercise but as a practical and regulatory necessity. Adoption has grown from fewer than 5 publications per year before 2017 to over 100 per year by 2022–2024, with SHAP, attention mechanisms, and LIME as the dominant techniques [40]. The FDA's January 2025 draft guidance, while limited to regulatory decision support, signals the direction of travel [41]. More importantly, explainability builds clinical trust: a physician will not override standard-of-care treatment based on an unexplainable AI prediction, regardless of the model's reported AUC.
...
[44] Nelson MR, Tipney H, Painter JL, et al. The support of human genetic evidence for approved drug indications. *Nature Genetics*. 2015;47(8):856–860.

[45] Elmentaite R, Kumasaka N, Roberts K, et al. Cells of the human intestinal tract mapped across space and time. *Nature*. 2021;597:250–255.
